...
- 背书模式只有确认到本人声纹才清零静默计时；用户端唯一可调检测参数是 20～60 秒的未检测到本人提醒时间，学习中允许修改且不得清除已经累计的未确认时长或在途声纹候选。
- 背书声音链路固定为“约 3 秒自动环境适应 → 高召回 VAD 分段 → CAM++ 声纹终判”。VAD 内部余量固定为 8 dB，不持久化、不向用户暴露；若最初约 3 秒已被朗读污染，自动噪声基线必须使用保守上限，适应结束后疑似语音候选须在 300 ms 内恢复。VAD 只筛选候选，不等同于本人判断。
- 自习模式不要求声纹，完全不创建或使用 `AdaptiveVad`，不做环境底噪校准，也不使用音量/sensitivity 门槛筛选；麦克风打开后直接用本地 CED Mini 对最近约 2 秒滚动窗口分类并约每 1 秒更新，首次有效分类约需 2 秒。人声专用阈值固定 0.12，音乐/通用媒体阈值固定 0.20；键盘单独不得累计。已形成媒体候选后，孤立阴性窗口不得立即清零，必须连续正常 5 秒才清除；短暂停顿后继续播放沿用原累计。用户只调 3～15 秒持续时间。
- 自习模式请求麦克风时应关闭 `echoCancellation`、`noiseSuppression` 与 `autoGainControl`；必须检测驱动实际设置并在驱动拒绝时提示，不能把请求关闭描述为系统必然遵守。
- 待命时允许在检测面板启动“当前设置测试”：必须复用正式检测链路；背书自动适应环境后做 VAD/CAM++ 验证，自习直接形成首个分类窗口。测试不启动会话、有效时钟、场景计划、音轨、提醒或学习记录；停止、开始学习、切换模式、录入声纹、最小化/隐藏窗口和退出时必须释放测试麦克风。
- 用户界面不得出现底噪/抗噪滑块、白色门槛线、漂浮窗底噪条或手动重新校准。主界面音量活动条只用于确认麦克风收到声音，不得作为检测门槛；漂浮窗只显示判断结果、动画和悬停计时/操作。旧设置中的 `reciteSensitivityDb` 必须在载入时忽略，并在下次保存设置时移除。
- 正式学习中修改持续时间只更新门槛，不得清除已经累计的候选；待命预检的数据是一次性测试状态，修改后可清除预检累计并按新时长重新测试。自习没有 sensitivity 设置。
...
- 后台方式为 `hidden | floating`，默认完全隐藏。漂浮模式必须复用同一个主窗口、`webContents`、Canvas、动画播放器与麦克风流；默认/最大 320×225，最小 224×170，允许用户缩放并保存尺寸。常态只显示当前判断和小动画；只有进入红色异常状态后，才在原判断中附带累计时间，不得另设常驻异常计时。鼠标悬停后显示“已学习”时长、“隐藏”和“放大”；“隐藏”的实际行为是完全隐藏到后台。状态区、悬停计时区和动画区可拖动，按钮必须保持可点击；取消双击放大，只允许点“放大”。
...
- 禁止生成、替换、错配或叠加无来源音效。
- 普通无声巡查动画继续检测；只有开场、结束、违规、表扬等带源音轨的动画暂停检测和有效学习计时，应用自己的音轨不能计为本人背书。
- 休息后的有声开场结束时，背书模式重新进行约 3 秒自动环境适应；自习模式直接恢复分类并重新形成约 2 秒首窗，不增加环境适应阶段。
...
- `%APPDATA%\背书自习监督` 是为兼容既有安装而保留的稳定旧数据根，可能包含真实用户数据。可见产品名改为“凛冬督学局”后也不得迁移或改名，否则升级用户可能丢失声纹或被要求重复录入。没有用户明确授权时，不读取声纹内容、不删除、不移动、不覆盖。
- `window-preferences.json` 只允许保存 `backgroundMode` 与 `floatingWindowSize`，不得保存窗口位置、检测结果、学习时长、窗口文字、音频、声纹或个人信息；非法模式回退 `hidden`，非法尺寸钳制到 224×170～320×225。
- `study-preferences.json` 只允许保存模式、两个提醒时长和用户明确选择的麦克风 `deviceId`/`label`；不得保存音频、声纹、检测结果或学习记录。设备缺失时保留原选择并要求用户手动重选，禁止按名称或系统默认设备静默替换。
...
- 安装版运行期间出现多个 `凛冬督学局.exe` 是 Electron 渲染、GPU 等辅助进程，不等于磁盘上存在多个安装版或持久化缓存。
