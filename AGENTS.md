...
- 产品名固定为“背书自习监督”；当前本地版本为 `1.12.0`，交付物为 `背书自习监督-安装版-1.12.0.exe` 与 `背书自习监督-便携版-1.12.0.exe`。
...
- 自习模式不要求声纹，完全不创建或使用 `AdaptiveVad`，不做环境底噪校准，也不使用音量/sensitivity 门槛筛选；麦克风打开后直接用本地 CED Mini 对最近约 2 秒滚动窗口分类并约每 1 秒更新，首次有效分类约需 2 秒。连续阳性累计必须抵扣窗口重叠，任一阴性分类窗口必须立即清空候选；用户只调 3～15 秒持续时间。单独键盘证据不得累计，键盘占主导时必须保留“键盘 + 至少两项媒体标签”的双证据路径。
- 自习模式请求麦克风时应关闭 `echoCancellation`、`noiseSuppression` 与 `autoGainControl`；必须检测驱动实际设置并在驱动拒绝时提示，不能把请求关闭描述为系统必然遵守。
- 待命时允许在检测面板启动“当前设置测试”：必须复用正式检测链路；背书先校准底噪，自习直接形成首个分类窗口。测试不启动会话、有效时钟、场景计划、音轨、提醒或学习记录；停止、开始学习、切换模式、录入声纹、最小化/隐藏窗口和退出时必须释放测试麦克风。
- 常驻音量条和详细音量条的白色门槛线只属于背书模式，必须与背书 sensitivity range、ARIA、持久化设置和 VAD 双向同步。自习模式的只读音量活动条只展示原始 RMS，必须隐藏门槛线、抗噪滑块与重新校准按钮，禁止让该数值参与 CED 分类判定。
- 自习预检或正式学习修改持续时间时必须清除旧 `QuietModeDetector` 累计/锁定、音频事件滚动缓冲和 `preflightThresholdReached`，再等待约 2 秒首窗并按新时长累计；自习没有 sensitivity 设置。
...
- 后台方式为 `hidden | floating`，默认完全隐藏。漂浮模式必须复用同一个主窗口、`webContents`、Canvas、动画播放器与麦克风流，固定 320×225；常态只显示统一检测结果和 16:9 动画，悬停后才显示有效学习计时、完全隐藏和放大操作。
- 非致命提醒必须按来源返回 `scene / hidden / floating`；提醒返回状态由主进程持有并用递增编号防旧请求，渲染器不得自由指定任意目标窗口模式。致命提醒、结束与异常清理回 `scene`。
...
- 休息后的有声开场结束时，背书模式重新做约 3 秒底噪校准；自习模式直接恢复分类并重新形成约 2 秒首窗，不增加底噪校准阶段。
...
- `window-preferences.json` 只允许保存 `hidden | floating`，不得加入检测结果、学习时长、窗口文字、音频或个人信息；损坏值回退为 `hidden`。
