...
- 当前可见产品名固定为“凛冬督学局”；当前源码版本为 `1.13.0`。1.13.0 候选交付物、大小与 SHA-256 在隔离构建前均为待生成，不得沿用 1.12.0 数值。
...
- 后台方式为 `hidden | floating`，默认完全隐藏。漂浮模式必须复用同一个主窗口、`webContents`、Canvas、动画播放器与麦克风流；默认/最大 320×225，最小 224×170，允许用户缩放并保存尺寸。状态区、计时区和动画区可拖动，按钮必须保持可点击；取消双击放大，只允许点“放大”。
