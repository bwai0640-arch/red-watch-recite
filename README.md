...
当前本地版本为 1.12.0，发布日期为 2026-07-22。
...
- 检测面板可选择“完全隐藏”或“漂浮窗”。漂浮窗复用同一个主窗口，只显示检测结果和 16:9 小动画；鼠标移入时才显示“已学习 00:00”、隐藏和放大。违规会把同一窗口恢复为完整场景，提醒结束后回到原后台方式。只有休息券使用一个右下角独立提示窗。
- 场景按 1920×1080 等比例展示，屏幕不足时整体缩小，不拉伸源动画。
- 主窗口可拖动和缩放；右上角依次为最小化、最大化/还原、隐藏到后台，双击标题栏也可最大化/还原。完整场景安全最小尺寸为 960×540；漂浮模式固定为 320×225、置顶且不显示任务栏按钮。
