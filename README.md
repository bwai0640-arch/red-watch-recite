...
当前本地版本为 1.11.0，发布日期为 2026-07-21。
...
- 自习模式不要求声纹；本地 CED Mini 对最近约 2 秒声音做滚动识别，约每 1 秒更新一次。连续窗口会先抵扣约 1 秒重叠，避免同一小段声音被重复计时；单独键盘声不累计，键盘与媒体标签同时出现时仍按媒体证据判断。提醒阈值可在 3～15 秒内调节。
...
公开仓库包含运行与构建所需媒体、图标和模型，但不包含 `work/speaker-fixtures/` 中的本地真人测试语音，也不提交音频事件模型的临时正样本。依赖真人 fixture 的声纹模型/UI 测试只供维护者在合规数据准备完成后运行。1.11.0 的本轮自动验证仅运行后台纯 Node 测试，不启动 Electron、不显示窗口、不使用真实麦克风；已经按 [测试说明](docs/TESTING.md) 通过临时音乐、人声与真实键盘 fixture 的模型正样本门禁，fixture 未提交、未打包，测试后从临时目录清理。
