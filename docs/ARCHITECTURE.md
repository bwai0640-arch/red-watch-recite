...
- `floating`：同一窗口默认/最大 320×225、最小 224×170，可缩放、置顶并跳过任务栏；上方 44 像素常驻镜像统一检测结果与独立异常累计，下方继续显示同一 Canvas 的 16:9 动画。除操作按钮外全区域为原生拖动区；鼠标移入时才在动画上方出现“已学习”有效计时、完全隐藏和恢复完整场景操作，不覆盖常驻状态。取消漂浮窗双击放大，只保留明确的“放大”按钮。
...
漂浮窗的 `will-move` 只标记一次手动移动，`moved` 再读取 Windows 原生拖动完成后的最终 bounds；移动链路不调用 `preventDefault()` 或 `setBounds()`，因此不会卡边，也不会用旧宽高覆盖用户刚调整的尺寸。位置只在当前进程用于休息提示避让后返回，不写入偏好文件。用户缩放结束时，`will-resize`/`resized` 把尺寸钳制在 224×170～320×225，并串行保存最后尺寸；快速连续选择后台方式时仍以最后一次操作为准。
