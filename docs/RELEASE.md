...
cd D:\RedWatchRecite
$candidate = 'work\release-candidate-1.13.3'
...
`D:\RedWatchRecite\work\release-candidate-<version>\凛冬督学局-安装版-<version>.exe`

候选便携版应为：

`D:\RedWatchRecite\work\release-candidate-<version>\凛冬督学局-便携版-<version>.exe`

electron-builder 还会在隔离候选目录生成 `win-unpacked\凛冬督学局.exe`；本轮不运行 UI，只做包内容检查，完成后不把该目录复制到交付目录。
...
- 后台门禁至少包含 `window-mode-policy-test.cjs`、`floating-window-source-test.cjs` 与 `adversarial-user-simulation-test.cjs`：严格核对后台偏好、提醒编号、同主窗口、漂浮拖缩、菜单、黄色动画状态和刁难用户场景。
...
- 验证 `speakerVerificationPending` 时提醒入口无条件返回；渲染侧单次 CAM++ 复核超过 5 秒应安全停止且不计违规，不能等待 Worker 30 秒保护期间先播放处罚动画。
...
- 验证 `study-preferences.json` 除固定格式版本外只含 `mode`、20～60 秒、3～15 秒和选定麦克风 `deviceId`/`label`；额外字段必须拒绝，原子写入后不得出现音频、声纹、检测结果、学习记录或计时。已选设备缺失时必须保留原 ID/标签并提示，不能按同名设备或系统默认设备自动替换。
...
$artifacts = Get-Item -LiteralPath `
  'D:\RedWatchRecite\release-staging\凛冬督学局-安装版-<version>.exe', `
  'D:\RedWatchRecite\release-staging\凛冬督学局-便携版-<version>.exe'
